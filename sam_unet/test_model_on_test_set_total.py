...
def test_one_epoch(args, model, test_loader, logger):
    ...
    for batch, dict_list in enumerate(test_loader):
        ...
        # test_batch_metrics = sum([SegMetrics(out[i]["masks"], dict_list[i]["labels"], args.metrics) for i in range(batch_size) if out[i]["masks"].shape == dict_list[i]["labels"].shape]) / batch_size
        if int(batch+1) % 50 == 0:
